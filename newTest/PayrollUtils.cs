...
namespace newTest
{
    public class PayrollUtils
    {
        ...
        public void GeneratePayrollPdf(Employee User,string directory, DateTime DtStart, DateTime DtEnd)
        {
            try
            {
                // Configurando PDF
                ...
                string documentName = directory == "Selecione o diretório" ? $"D:\\Downloads(D)\\demonstrativo-{DateTime.Now.ToString("yyyy-MM-dd")}-name.pdf" : directory + $"\\demonstrativo-{DateTime.Now.ToString("yyyy-MM-dd")}-name.pdf";
                ...
                cells = new PdfPCell(new Phrase(DtEnd.ToString("MM/yyyy")));
                cells.Border = PdfPCell.BOTTOM_BORDER | PdfPCell.LEFT_BORDER | PdfPCell.RIGHT_BORDER;
                table.AddCell(cells);
                ...
                int workedHours = GetWorkedHours(User, DtStart, DtEnd);
        ...
        private int GetWorkedHours(Employee User, DateTime DtStart, DateTime DtEnd)
        {
            DateTime startDate = DtStart;
            DateTime endDate = DtEnd;
            ...
            {
                hours += GetDateHours(date, User);
            }

            return hours;
        }

        private int GetDateHours(DateTime date, Employee User)
        {
